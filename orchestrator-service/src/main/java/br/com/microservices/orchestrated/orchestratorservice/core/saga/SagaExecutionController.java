...
public class SagaExecutionController {
    ...
    public ETopics getNextTopic(Event event) {
        ...
        var topic =  findBySourceAndStatus(event);
        logCurrentSaga(event, topic);
        return topic;
    }

    private ETopics findBySourceAndStatus(Event event) {
    ...
    private void logCurrentSaga(Event event, ETopics topic) {
        ...
        switch (event.getStatus()) {
            ...
            case ROLLBACK_PENDING -> log.info("### CURRENT SAGA: {} | SENDING TO ROLLBACK CURRENT SERVICE " +
                            "| NEXT TOPIC {} | {}",
                    source, topic, sagaId);

            case FAIL -> log.info("### CURRENT SAGA: {} | SUCCESS | SENDING TO ROLLBACK PREVIOUS SERVICE {} | {}",
                    source, topic, sagaId);
        }
    }
